...
class Vertex(object):
    ...
    def snd_msg(self, neigh):
        ...
        if self._observed:
            ...
            m_tag = np.zeros((256))
            m_tag += phi_mat[self._y][:]

        #elif len(self._in_msgs) != len(self._neighs):
            ...
            m_tag = np.add(phi_mat, m_tag_single_entry.reshape(-1, 1))
            m_tag = np.max(m_tag, axis=1)

            # Normalized - calc m
            # max_exp = np.max(m_tag)
...
if __name__ == '__main__':
    ...
    misc.toimage(image_final).save(outfile_name +  str(ITERATIONS) + '.png')
